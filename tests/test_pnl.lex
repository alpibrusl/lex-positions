...
fn test_unrealized_flat_is_zero() -> Result[Unit, Str] {
  assert_true(d.is_zero(pnl.unrealized_pnl(pos.flat(key()), price(1000, -2))), "unrealized flat = 0")
}

fn test_unrealized_long_gain() -> Result[Unit, Str] {
  let p := pu.apply_fill(pos.flat(key()), { exec_id: "E1", qty: 100, price: price(1000, -2), is_buy: true })
...
fn test_unrealized_short_gain() -> Result[Unit, Str] {
  let p := pu.apply_fill(pos.flat(key()), { exec_id: "E1", qty: 50, price: price(2000, -2), is_buy: false })
...
fn test_total_pnl() -> Result[Unit, Str] {
  let p0 := pu.apply_fill(pos.flat(key()), { exec_id: "E1", qty: 100, price: price(1000, -2), is_buy: true })
  let p1 := pu.apply_fill(p0, { exec_id: "E2", qty: 50, price: price(1100, -2), is_buy: false })
  let total := pnl.total_pnl(p1, price(1050, -2))
  let expected := d.add(d.mul(price(100, -2), d.from_int(50)), d.mul(price(50, -2), d.from_int(50)))
  assert_eq_dec(total, expected, "total pnl = 75.00")
}

fn suite() -> List[Result[Unit, Str]] {
  [test_unrealized_flat_is_zero(), test_unrealized_long_gain(), test_unrealized_long_loss(), test_unrealized_short_gain(), test_total_pnl()]
}
