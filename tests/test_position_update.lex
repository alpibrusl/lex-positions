...
fn test_second_buy_waac() -> Result[Unit, Str] {
  ...
  # WAAC = (100*10.00 + 100*11.00)/200 = 10.50 at 8dp
  match assert_eq_int(p1.qty, 200, "qty=200") {
    Err(e) => Err(e),
    Ok(_)  => assert_eq_dec(p1.avg_cost,
      { coefficient: 1050000000, exponent: -8 }, "avg=10.50 at 8dp"),
  }
}
...
fn test_second_sell_waac() -> Result[Unit, Str] {
  ...
  # WAAC = (50*20.00 + 50*22.00)/100 = 21.00 at 8dp
  match assert_eq_int(p1.qty, 0 - 100, "qty=-100") {
    Err(e) => Err(e),
    Ok(_)  => assert_eq_dec(p1.avg_cost,
      { coefficient: 2100000000, exponent: -8 }, "avg=21.00 at 8dp"),
  }
}
...
fn test_partial_sell_realizes_pnl() -> Result[Unit, Str] {
  ...
  # realized = (11.00 - 10.00) * 40 = 40.00 = mul({coefficient:100,exponent:-2}, from_int(40))
...
fn test_cross_zero_sell_flips_to_short() -> Result[Unit, Str] {
  ...
  # close 100 longs; realized = 100.00; open 50 short @ 11.00
...
fn test_cross_zero_buy_flips_to_long() -> Result[Unit, Str] {
  ...
  # cover 50; realized = (20.00-18.00)*50 = 100.00; open 30 long @ 18.00
...
fn test_pnl_accumulates() -> Result[Unit, Str] {
  ...
  # realized1 = 0.50*50=25.00; realized2 = 1.00*50=50.00; total=75.00
...
fn suite() -> List[Result[Unit, Str]] {
  [
    ...
    test_second_buy_waac(),
    test_second_sell_waac(),
    test_partial_sell_realizes_pnl(),
    test_partial_buy_realizes_pnl_on_short(),
    test_exact_close_resets_position(),
    test_cross_zero_sell_flips_to_short(),
    test_cross_zero_buy_flips_to_long(),
