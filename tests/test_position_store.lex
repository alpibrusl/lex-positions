    ...
    Ok(db)   => {
      ...
      match store.store(db, position) {
        ...
        Ok(_)  => match store.fetch(db, k) {
          Err(e) => fail(dbe.message(e)),
          Ok(p)  =>
            match assert_eq_int(p.qty, 100, "qty=100") {
              Err(e) => Err(e),
      ...
      match store.apply_and_store(db, k, f1) {
        ...
        Ok(_)  => match store.apply_and_store(db, k, f2) {
          ...
          Ok(p)  =>
            match assert_eq_int(p.qty, 100, "qty=100") {
              Err(e) => Err(e),
              Ok(_)  => assert_eq_dec(p.avg_cost,
                { coefficient: 2050000000, exponent: -8 }, "avg=20.50 at 8dp"),
            },
        },
      }
    },
...
fn test_two_accounts_isolated() -> [positions, sql, fs_write] Result[Unit, Str] {
  match open_db() {
    ...
    Ok(db)   => {
      ...
      let _ := store.apply_and_store(db, k1, f1)
      let _ := store.apply_and_store(db, k2, f2)
      match store.fetch(db, k1) {
        Err(e) => fail(dbe.message(e)),
        Ok(p1) => match store.fetch(db, k2) {
          Err(e) => fail(dbe.message(e)),
          Ok(p2) =>
            match assert_eq_int(p1.qty, 200, "acc-a qty=200") {
              Err(e) => Err(e),
...
fn suite() -> [positions, sql, fs_write] List[Result[Unit, Str]] {
  [
    test_fetch_empty_returns_flat(),
    test_store_and_fetch_roundtrip(),
    test_apply_and_store_two_buys(),
    test_apply_and_store_realizes_pnl(),
    test_remove_leaves_flat(),
    test_two_accounts_isolated(),
  ]
}
