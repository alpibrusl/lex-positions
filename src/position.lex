...
type PositionKey = { account :: Str, symbol :: Str }

type Position = { key :: PositionKey, qty :: Int, avg_cost :: d.Decimal, realized_pnl :: d.Decimal }

type Fill = { exec_id :: Str, qty :: Int, price :: d.Decimal, is_buy :: Bool }

# ---- Constructors ---------------------------------------------------
fn flat(key :: PositionKey) -> Position
  examples {
    flat({ account: "ACC1", symbol: "AAPL" }) => { key: { account: "ACC1", symbol: "AAPL" }, qty: 0, avg_cost: { coefficient: 0, exponent: 0 }, realized_pnl: { coefficient: 0, exponent: 0 } }
  }
{
...
fn is_long(pos :: Position) -> Bool
  examples {
    is_long({ key: { account: "A", symbol: "X" }, qty: 10, avg_cost: { coefficient: 0, exponent: 0 }, realized_pnl: { coefficient: 0, exponent: 0 } }) => true,
    is_long({ key: { account: "A", symbol: "X" }, qty: 0, avg_cost: { coefficient: 0, exponent: 0 }, realized_pnl: { coefficient: 0, exponent: 0 } }) => false
  }
{
  pos.qty > 0
}
...
fn is_short(pos :: Position) -> Bool
  examples {
    is_short({ key: { account: "A", symbol: "X" }, qty: 0 - 5, avg_cost: { coefficient: 0, exponent: 0 }, realized_pnl: { coefficient: 0, exponent: 0 } }) => true
  }
{
  pos.qty < 0
}
...
fn is_flat(pos :: Position) -> Bool
  examples {
    is_flat({ key: { account: "A", symbol: "X" }, qty: 0, avg_cost: { coefficient: 0, exponent: 0 }, realized_pnl: { coefficient: 0, exponent: 0 } }) => true
  }
{
  pos.qty == 0
}
...
fn parse_price(s :: Str) -> Option[d.Decimal] {
  ...
  if list.is_empty(parts) {
    ...
    if list.len(parts) == 1 {
      ...
      let int_s := match list.head(parts) {
        None => "0",
        Some(x) => x,
      }
      let frac_s := match list.head(list.tail(parts)) {
        None => "0",
        Some(x) => x,
      }
