...
fn init(db :: conn.ConnDb) -> [positions, sql] Result[Unit, dbe.DbErr] {
...
fn fetch(db :: conn.ConnDb, key :: pos.PositionKey) -> [positions, sql] Result[pos.Position, dbe.DbErr] {
  let sq := q.for_dialect(
    { sql: "SELECT qty, avg_cost_str, realized_pnl_str FROM positions WHERE account = ? AND symbol = ?",
      params: [PStr(key.account), PStr(key.symbol)] },
    db.dialect
  )
  let raw :: Result[List[{ qty :: Int, avg_cost_str :: Str, realized_pnl_str :: Str }], SqlError] :=
...
fn store(db :: conn.ConnDb, position :: pos.Position) -> [positions, sql] Result[Unit, dbe.DbErr] {
  let avg_str := pos.decimal_to_str(position.avg_cost)
  let pnl_str := pos.decimal_to_str(position.realized_pnl)
  let sq := q.for_dialect(
    { sql: "INSERT INTO positions (account, symbol, qty, avg_cost_str, realized_pnl_str) VALUES (?, ?, ?, ?, ?) ON CONFLICT (account, symbol) DO UPDATE SET qty = EXCLUDED.qty, avg_cost_str = EXCLUDED.avg_cost_str, realized_pnl_str = EXCLUDED.realized_pnl_str",
      params: [
        PStr(position.key.account),
        PStr(position.key.symbol),
        PInt(position.qty),
        PStr(avg_str),
        PStr(pnl_str),
      ] },
...
fn apply_and_store(
  ...
  key  :: pos.PositionKey,
  fill :: pos.Fill
) -> [positions, sql] Result[pos.Position, dbe.DbErr] {
  match fetch(db, key) {
    Err(err)    => Err(err),
    Ok(current) => {
...
fn remove(db :: conn.ConnDb, key :: pos.PositionKey) -> [positions, sql] Result[Unit, dbe.DbErr] {
  let sq := q.for_dialect(
    { sql: "DELETE FROM positions WHERE account = ? AND symbol = ?",
      params: [PStr(key.account), PStr(key.symbol)] },
    db.dialect
  )
  match sql.exec(db.handle, sq.sql, sq.params) {
...
fn decode_row(
  key :: pos.PositionKey,
  row :: { qty :: Int, avg_cost_str :: Str, realized_pnl_str :: Str }
) -> Result[pos.Position, dbe.DbErr] {
